...
for i in range(data5nmsc.shape[0]):
    data5nmsc[i, 4] /= 1e9
y_5nm = data5nmsc[:,5]
X_train_14nm, X_test_14nm, y_train_14nm, y_test_14nm = train_test_split(X_14nm, y_14nm, test_size=0.2, random_state=42)
...
y_train_scaled_14nm = scaler1.fit_transform(y_train_14nm.reshape(-1, 1)).flatten()
y_test_scaled_14nm = scaler1.transform(y_test_14nm.reshape(-1, 1)).flatten()
y_train_scaled_5nm = scaler1.transform(y_train_5nm.reshape(-1, 1)).flatten()
...
model_14nm.fit(X_train_scaled_14nm, y_train_scaled_14nm, epochs=500, batch_size=32, validation_data=(X_test_scaled_14nm, y_test_scaled_14nm), verbose=0)
...
transfer_model.fit(X_train_scaled_5nm, y_train_scaled_5nm, epochs=800, batch_size=16, verbose=0)
y_pred_scaled_5nm = transfer_model.predict(X_test_scaled_5nm)
...
print(MEDAE)
print(MAPE)
relative_bias = np.mean((y_pred_5nm.squeeze() - y_test_5nm.squeeze()) / y_test_5nm.squeeze()) * 100
relative_bias_mape = np.mean(np.abs((y_pred_5nm.squeeze() - y_test_5nm.squeeze()) / y_test_5nm.squeeze())) * 100
